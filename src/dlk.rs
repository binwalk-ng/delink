    ...
    match encrypted_data.get(0..ENCRYPTED_MAGIC.len()) {
        ...
        Some(header1_magic) => {
            if header1_magic == ENCRYPTED_MAGIC {
                ...
                let Some(signature_size_bytes) =
                    encrypted_data.get(signature_size_start..signature_size_end)
                else {
                ...
                let Some(header2_magic) = encrypted_data
                ...
                if header2_magic == ENCRYPTED_MAGIC {
                    ...
                    let Some(block_size_bytes) =
                        encrypted_data.get(block_size_start..block_size_end)
                    else {
                    ...
                    let Some(total_size_bytes) =
                        encrypted_data.get(total_size_start..total_size_end)
                    else {
                    ...
                    let total_data_size =
                        u32::from_le_bytes(total_size_bytes.try_into().unwrap()) as usize;

                    // Calculate the start and end offsets of the array of encrypted blocks
                    let cipher_data_start = next_header_offset + HEADER_SIZE;
                    let cipher_data_end = cipher_data_start + total_data_size;

                    // Get the bytes of all encrypted blocks
                    let Some(block_data) = encrypted_data.get(cipher_data_start..cipher_data_end)
                    else {
                        debug!(
