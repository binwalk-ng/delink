...
pub fn decrypt(encrypted_image: &[u8]) -> Result<Vec<u8>, DecryptError> {
    ...
    const KNOWN_FIRMWARES: &[EncimgFirmware] = &[
        EncimgFirmware {
            name: "DAP-1665",
            image_sign: ImageSign::Value(b"wapac25_dlink.2015_dap1665"),
            encrypted_data_offset: 0,
        },
        EncimgFirmware {
            name: "DIR-822",
            image_sign: ImageSign::Value(b"wrgac43s_dlink.2015_dir822c1"),
            encrypted_data_offset: 0,
        },
        EncimgFirmware {
            name: "DIR-842",
            image_sign: ImageSign::Value(b"wrgac65_dlink.2015_dir842"),
            encrypted_data_offset: 0,
        },
        EncimgFirmware {
            name: "DIR-850L A1",
            image_sign: ImageSign::Value(b"wrgac05_dlob.hans_dir850l"),
            encrypted_data_offset: 0,
        },
        EncimgFirmware {
            name: "DIR-850L B1",
            image_sign: ImageSign::Value(b"wrgac25_dlink.2013gui_dir850l"),
            encrypted_data_offset: 0,
        },
        ...
        EncimgFirmware {
            name: "DIR-880L",
            image_sign: ImageSign::Value(b"wrgac16_dlink.2013gui_dir880"),
            encrypted_data_offset: 0,
        },
        ...
        EncimgFirmware {
            name: "DIR-885L",
            image_sign: ImageSign::Value(b"wrgac42_dlink.2015_dir885l"),
            encrypted_data_offset: 0,
        },
        ...
        EncimgFirmware {
            name: "DAP-1720",
            image_sign: ImageSign::Value(b"wapac28_dlink.2015_dap1720"),
            encrypted_data_offset: 0,
        },
        EncimgFirmware {
            name: "DIR-2610",
            image_sign: ImageSign::Offset(0),
    ...
    for firmware in KNOWN_FIRMWARES {
        // Get the image_sign
        let image_sign: &[u8] = match firmware.image_sign {
            ImageSign::Value(value) => value,
            ImageSign::Offset(image_sign_offset) => {
                match encrypted_image.get(image_sign_offset..) {
                    None => &[],
                    Some(image_sign_bytes) => {
                        let len = image_sign_bytes
                            .iter()
                            .position(|b| *b == b'\0')
                            .unwrap_or(image_sign_bytes.len());
                        &image_sign_bytes[..len]
                    }
                }
            }
        };
        ...
        if let Some(encrypted_data) = encrypted_image.get(firmware.encrypted_data_offset..) {
            ...
            let result = keys.iter().find_map(|crypto| {
                ...
                {
                    debug!("Decryption OK");
                    return aes_256_cbc_decrypt_unpadded(encrypted_data, &crypto.key, &crypto.iv)
                        .ok();
                }
                None
            });
...
pub fn keygen(image_sign: &[u8]) -> Vec<EncimgKey> {
    ...
    const PROG_BOARD_FWS: &[[u8; PROG_BOARD_LEN]] = &[
        // Device(s): DAP-1665
    ...
    let mut keys: Vec<EncimgKey> = PROG_BOARD_FWS
        .iter()
        .map(|fw| {
            let iv_seed = &fw[IV_SEED_START..IV_SEED_END];
            let key_seed = &fw[KEY_SEED_START..KEY_SEED_END];
...
pub fn encrypt_xor(image_sign: &[u8], data: &[u8]) -> Vec<u8> {
    ...
    let mut sign_bytes = image_sign.iter().copied().cycle();
    data.iter()
        .map(|&data_byte| {
            let sign_byte = sign_bytes.next().unwrap();
            let res = data_byte ^ sign_byte ^ xor_byte;
            xor_byte += 1;
