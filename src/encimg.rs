        ...
        if let Some(encrypted_data) = encrypted_image.get(firmware.encrypted_data_offset..) {
            ...
            let result = keys.iter().find_map(|crypto| {
                if let Some(first_block) = encrypted_data.get(..16)
                    ...
                    && first_block.len() >= MAGIC_LEN
                    && known_decrypted_magics
                        .iter()
                        .any(|m| first_block[..MAGIC_LEN] == *m.as_slice())
                {
                    debug!("Decryption OK");
...
pub fn keygen(image_sign: &[u8]) -> Vec<EncimgKey> {
    ...
    let mut keys: Vec<EncimgKey> = Vec::with_capacity(prog_board_fws.len());
